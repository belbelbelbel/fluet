        ...
        <div className="w-full max-w-md">
          {(title || subtitle) && (
            <div className="mb-6 text-center">
              {title && (
                <h1 className="text-xl font-medium text-foreground dark:text-white">
                  {title}
                </h1>
              )}
              {subtitle && (
  ...
  return (
    <AuthPageShell>
      <div className="text-center py-12">
        <div
          className="w-8 h-8 border-2 border-border border-t-gray-800 dark:border-slate-600 dark:border-t-slate-200 rounded-full animate-spin mx-auto mb-4"
          aria-hidden
        />
        <p className="text-sm text-muted-foreground dark:text-slate-400">{message}</p>
      </div>
    </AuthPageShell>
  );
}
